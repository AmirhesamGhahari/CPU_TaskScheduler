...
/**
 * 
 * Nonpreemptive shortest job first scheduling class 
 * 
 */
public class Nonpreemptive_SJF {
	 ...
	 */
	private ArrayList<Nonpreemptive_SJF.Process> computationList;
	
	/**
	 * linked list object responsible for holding the I/O task burst processes
	 */
	private LinkedList<Nonpreemptive_SJF.Process> IOList;

	/**
	 * linked list object responsible for holding the already processed and terminated processes
	 */
	private LinkedList<Nonpreemptive_SJF.Process> processedProcesses;;
	 ...
	 */
	public Nonpreemptive_SJF() {
		
		this.computationList =  new ArrayList<Nonpreemptive_SJF.Process>();
		this.IOList = new LinkedList<Nonpreemptive_SJF.Process>();
		this.processedProcesses = new LinkedList<Nonpreemptive_SJF.Process>();
	}
	 ...
	 */
	public LinkedList<Nonpreemptive_SJF.Process> getProcessedlist()
	{
	...
	public long[] processScheduling()
	{
		...
		while(this.computationList.size() != 0 || this.IOList.size() != 0) 
		{
			...
			{
				long tless = this.computationList.get(0).computationSequence.getFirst();
				for(Process pr : this.computationList)
				{
					if(pr.computationSequence.getFirst() < tless)
					{
						tless = pr.computationSequence.getFirst();
					}
				}
				...
				{
					if(this.computationList.get(j).computationSequence.getFirst() == tless)
					{
						ind = j;
					}
				}
			...
			if(this.IOList.size() == 0)
			{
				...
				} else 
				{
					...
					int numOfElem = j22 - 1;
					for( int rw1 = 0; rw1 < numOfElem + 1; rw1++)
					{
